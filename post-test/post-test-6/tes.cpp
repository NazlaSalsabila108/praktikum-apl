            ...
            while (true) {
                ...
                if (role == "admin") {
                    if (pilihan == 1) {
                        ...
                        sortPasien(dataPasien, hitungPasien); 
...
void sortPasien(Pasien *dataPasien, int hitungPasien) {
    sort(dataPasien, dataPasien + hitungPasien, [](const Pasien &a, const Pasien &b) {
        if (a.nama != b.nama) {
            return a.nama > b.nama; // Sort nama secara descending dari terbesar dulu baru terkecil 
        }
        return a.umur < b.umur; // Sort umur secara ascending yang dari terkecil dulu baru terbesar 
    });
}
...
void sortPasien(Pasien *dataPasien, int hitungPasien) {
    for (int i = 1; i < hitungPasien; i++) {
        ...
        while (j >= 0 && (dataPasien[j].nama < key.nama || 
            (dataPasien[j].nama == key.nama && dataPasien[j].umur > key.umur))) {
